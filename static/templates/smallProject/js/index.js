 $(function(){
    ...
    function ShowCountDown(year,month,day,divname) {
        ...
        cc.innerHTML ="<span id='h'>"+0+hour+"</span>"+"<span class='colon'>:</span>"+"<span id='m'>"+minute+"</span>"+"<span class='colon'>:</span>"+"<span id='s'>"+second+"</span>";
    ...
    // 底部最新购买纪录
    var $container = $('#latest-purchase-box'),
        $list = $('.latest-purchase-list'),
        $copyList = $('.latest-purchase-list-copy'),
        timer = null;
        $copyList.html($list.html());

    function roll(){
        var num = $container.scrollTop() ;
        if($list.outerHeight() > num){
            num ++
    ...
    // 切换尺寸
    $('.product-size').on('click','li',function(){
