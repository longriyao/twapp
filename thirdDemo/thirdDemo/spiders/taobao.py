...
class TaobaoSpider(scrapy.Spider):
    ...
    start_urls = ['http://taobao.com/'] #开始的网址

    def parse(self, response):    #第一步，进入网站，搜索关键词
        key = '女包'
        for i in range(0, 3):
            url = 'https://s.taobao.com/search?q=' + str(key) + '&s=' + str(44*i) + '&sort=sale-desc'
            yield Request(url=url, callback=self.page)  #返回网址和
        pass
