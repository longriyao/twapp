...
def index(req):
    ...
    if req.method == 'POST':
       ...
       return render(req, 'static/t543410226625/index1.html', {'res': res,'babyname': babyname})
...
def add(req):
  if req.method == 'POST':
   ...
   resa.save()
   colorurl='/products/t543410226625/'+color+'.jpg'
   return render(req, 'static/t543410226625/success.html', {'num':num,'color':color,'colorurl':colorurl,'money':money,'res':res,'babyname': babyname})
  else:
   num = req.COOKIES.get('num','')#获取购买的数量
   color=req.COOKIES.get('color','')#获取颜色
   money=req.COOKIES.get('money','')#获取总金额
   ...
   return render(req, 'static/t543410226625/payment.html', {'ordernum':ordernum, 'num':num,'color':color,'colorurl':colorurl, 'money':money,'babyname': babyname})
